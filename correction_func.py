...
class Correction_Filter():
    def __init__(self, s, scale_factor, x_shape, eps=0, r=None, inv_type='naive'):
        ...
        if r != None:
            ...
            self.r = util.get_bicubic(scale_factor).float().to(s.device)
    ...
    def find_H(self, s, r):
        R = util.fft_torch(r, self.shape)
        S = util.fft_torch(s, self.shape)

        R, S = util.shift_by(R, 0.5*(not self.scale_factor%2)), util.shift_by(S, 0.5*(not self.scale_factor%2))
        ...
        # Invert S*R
        Q_d_inv = util.dagger(Q_d, self.eps, mode=self.inv_type)
...
def est_corr(img_name, y_full, R_dag, S_conj, args, log_file=None, ref_bic=None, s_target=None):
    ...
    s_c = torch.fft.ifftn(util.fft_torch(s_d_1/s_d_1.sum(), y_full.shape[2:4])*util.fft_torch(s_d_2/s_d_2.sum(), y_full.shape[2:4])*
                          util.fft_torch(s_d_3/s_d_3.sum(), y_full.shape[2:4])*util.fft_torch(s_d_4/s_d_4.sum(), y_full.shape[2:4]) ,dim=(-2,-1))
    with torch.no_grad():
    ...
    for itr in range(args.iterations):
        ...
        x_hat1 = util.fft_Filter_(x_hat,  util.fft_torch(util.flip_torch(s_d_1)/s_d_1.sum(), s = x_hat.shape[2:4]))
        x_hat2 = util.fft_Filter_(x_hat1, util.fft_torch(util.flip_torch(s_d_2)/s_d_2.sum(), s = x_hat1.shape[2:4]))
        x_hat3 = util.fft_Filter_(x_hat2, util.fft_torch(util.flip_torch(s_d_3)/s_d_3.sum(), s = x_hat2.shape[2:4]))
        ...
        with torch.no_grad():
            x_c, y_c = util.get_center_of_mass(torch.roll(s_c.real, (s_c.shape[2]//2, s_c.shape[3]//2), dims=(-2,-1)), args.device)
        ...
        with torch.no_grad():
            if(args.save_trace and np.mod(itr, 10) == 0):
                util.save_img_torch(y_full_h, args.out_dir + 'within_loop.png')
